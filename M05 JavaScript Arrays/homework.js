...
function dePalabrasAFrase(palabras) {
   ...
   // Tu código:
   palabrasConectadas =palabras.join(' ');
   return palabrasConectadas;
}
...
function arrayContiene(array, elemento) {
   ...
   // Tu código:
   var elementoEnArray = array.includes(elemento) 
   
   if (elementoEnArray === true){
      return true;
   }
   else {
      return false;
...
let suma = 0 ;
for (let i=0; i<arrayOfNums.length; i++) {
   if(suma < Math.max(arrayOfNums[i])){
      suma = arrayOfNums[i];
   }
}
return suma;
...
function mesesDelAño(array) {
   ...
   array.forEach(cadena =>{
      if(cadena === "Enero" || cadena === "Marzo" || cadena === "Noviembre" )
      meses.push(cadena);
   });
